...
def extract_match_features(match, time_point=None):
    ...
    for player_index, player in enumerate(match['players']):
        ...
        feats += [
            ...
            (player_id + '_level',
             max([0] + [entry['level'] for entry in filter_events(player['ability_upgrades'], time_point)])),
    ...
    for team, team_players in (('radiant', radiant_players), ('dire', dire_players)):
        ...
        feats += [
            (
                '%s_first_ward_time' % team,
                min([entry['time'] for entry in team_wards]) if len(team_wards) > 0 else None),
        ]
